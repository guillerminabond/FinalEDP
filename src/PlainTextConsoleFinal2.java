...
public class PlainTextConsoleFinal2 {
	...
	public static void option3() {
		...
		for (int i = 0; i < entrada.size(); i++) {
			if(PacientesEnfermedades.get(extraerPaciente(entrada.get(i))).equals("LUPUS")) {
				pacL.add(entrada.get(i));
			}
			if(PacientesEnfermedades.get(extraerPaciente(entrada.get(i))).equals("HASHIMOTO")) {
				pacH.add(entrada.get(i));
			}
			if(PacientesEnfermedades.get(extraerPaciente(entrada.get(i))).equals("ADDISON")) {
